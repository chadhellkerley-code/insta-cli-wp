...
export default function InboxTab({ accounts, chats, agents }: InboxTabProps) {
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      const activeAccount = accounts.find(a => a.id === activeChat.accountId);
      ...
      if (activeAccount) {
        const response = await fetch("/api/whatsapp/send", {
          ...
          body: JSON.stringify({
            ...
            to: activeChat.contactPhone.replace(/\D/g, ''),
            messageType: "text",
            text: textToSend
          })
        });
        ...
        if (!response.ok) {
           ...
           console.error("Error calling whatsapp API", errData);
           throw new Error(errData.error || "Failed to send message via WhatsApp API");
        }
      }
      ...
      alert("Error al enviar el mensaje. Verifica que la cuenta de WhatsApp esté correctamente configurada.");
    }
  };
    ...
    try {
      ...
      if (activeAccount) {
        ...
        if (!response.ok) {
           const errData = await response.json();
           console.error("Error calling whatsapp API", errData);
           throw new Error(errData.error || "Failed to send attachment via WhatsApp API");
        }
      }
      ...
      alert("Error al enviar el archivo adjunto.");
    }
  };
